...
public class Main {
     ...
     */
	final static int NUM_HOMEWORKS = 5;

    /*
     * The number of homework assignments in the course.
     */
    final static int NUM_EXAMS = 3;
    ...
    public void exit(){
        try{
                GradebookWriter gradebookWriter = new GradebookWriter("gradebookOut.txt");
                gradebookWriter.writeGradebook(getRoster());
                gradebookWriter.close();
                System.exit(0);
        }catch(FileNotFoundException exception){
                getView().messageBox("Could not open gradebookOut.txt for writing. Exiting without saving.");
            System.exit(-1);
        }
    }
